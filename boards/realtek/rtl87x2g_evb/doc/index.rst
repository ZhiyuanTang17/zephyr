...
Overview
********
RTL87X2G-EVB contains a mother board known as the RTL87x2G Model B Evaluation Board, along with an interchangeable daughterboard that houses a real RTL87x2G series SoC.

.. image:: img/RTL87X2G_EVB_Blocks_Distribution_Diagram-Front.webp
     :align: center
     :alt: RTL87X2G_EVB_Blocks_Distribution_Diagram-Front
...
Essential Images
-------------------
To run a Zephyr application on the RTL87x2G, there are five essential images that need to be programmed into the board in addition to the Zephyr image. You can fetch these five images, the default flash layout, and a detailed explanation in `rtkconnectivity`_.
To download these images, MP Tool (for Windows) and MPCli Tool (for MAC/LINUX) developed by Realtek are recommended.

The principle of the download method for these two tools is via the UART interface. Note that P3_1 and P3_0 are required for using the UART downloading interface. To enter download mode for EVB model B, press the log key and the download mode key together and then press the reset key. After downloading, both keys need to be pressed again and released to enter normal mode.

The MP Tool download link is in the `RTL87x2g MP Tool Guide`_. The `RTL87x2g MP Tool Download Example`_ provides a specific step-by-step example for RTL87x2G image downloading using MP Tool.

The MPCli Tool download method is in the `RTL87x2g MPCli Tool Guide`_.

For flashing the Zephyr Application Image, we provide two methods:
...
Please refer to `J-Link Device Support Kit`_ for detailed instructions.

The flash loader and J-link Script for RTL87x2G can be found in `rtkconnectivity`_.

When writing the XML file, it is important to note that the XML format may vary with different versions of J-Link.
Ensure you are using the correct format compatible with your specific version of J-Link software.
Here is a reference template for the RTL87x2G XML file:

   .. code-block:: xml

    <Database>
        <Device>
            <ChipInfo Vendor="Realtek" Name="RTL87X2G" Core="JLINK_CORE_CORTEX_M55" WorkRAMAddr="0x00130000" WorkRAMSize="0x8000" />
            <FlashBankInfo Name="Flash Bank" BaseAddr="0x04000000" AlwaysPresent="1" >
            <LoaderInfo Name="CmsisSingleBank" Loader="RTL87X2G_FLASH.FLM" MaxSize="0x100000" LoaderType="FLASH_ALGO_TYPE_CMSIS" JLinkScriptFile="RTL87X2G.JLinkScript" />
            </FlashBankInfo>
        </Device>
    </Database>
...
.. figure:: img/EVB_model_B_SWD_Wiring_Full.webp
     :align: center
...
.. _RTL87x2g Introduction:
    https://www.realmcu.com/zh/Home/Product/RTL8762G-RTL877xG-Series

.. _RTL87x2g Documentation:
    https://www.realmcu.com/zh/Home/DownloadList/c175760b-088e-43d9-86da-1fc9b3f07ec3
...
.. _J-Link Device Support Kit:
    https://wiki.segger.com/J-Link_Device_Support_Kit
