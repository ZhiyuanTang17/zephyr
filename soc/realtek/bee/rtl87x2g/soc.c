...
static int rtl87x2g_platform_init(void)
{
	...
	init_osc_sdm_timer(); /* use osif timer api, os_register_pm_excluded_handle */
...
{
	/* rtl87x2g's cortex-m systick timer is using external clock source instead of cpu clock as
	 * referance. The priority of systick interrupt is lowest for rtl87x2g SoCs.
	 */
