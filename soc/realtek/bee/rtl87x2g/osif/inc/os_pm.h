...
#define US_TO_TICK_COUNT(us, residual)                                                             \
	cal_quotient_remainder(1000000 *                                                           \
				       (uint64_t)(os_sys_tick_clk_get() / os_sys_tick_rate_get()), \
			       os_sys_tick_clk_get(), us, &residual)
...
#define TICK_COUNT_TO_US(tick_count)                                                               \
	cal_quotient_remainder(os_sys_tick_clk_get(),                                              \
			       1000000 *                                                           \
				       (uint64_t)(os_sys_tick_clk_get() / os_sys_tick_rate_get()), \
			       tick_count, NULL)

#define SYSTICK_TO_US(systick) cal_quotient_remainder(os_sys_tick_clk_get(), 1000000, systick, NULL)
