...
static size_t mbedtls_internal_sha256_process_many(
...
{
    if (ctx->is224 == 0) {
        if (hw_sha256_cpu_update((void *)ctx, (uint8_t *)input, ilen) == false) {
...
{
    if (ctx->is224 == 0) {
        if (hw_sha256_finish((void *)ctx, (uint32_t *)output) == false) {
            // return non-zero to indicate error
            return 1;
        }
        return 0;
