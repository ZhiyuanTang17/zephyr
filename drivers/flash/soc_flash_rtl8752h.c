								...
								const void *data, size_t len)
{
	...
	bool is_src_in_flash = (src_addr >= FLASH_ADDR) &&
			       (src_addr < (FLASH_ADDR + FLASH_SIZE));
...
#else
	__ASSERT(!is_src_in_flash,
		"not supported: data in flash (0x%lx) cannot be used as source",
		(long)src_addr);
