				...
				size_t len)
{
	...
	bool is_src_in_flash = (src_addr >= FLASH_ADDR) && (src_addr < (FLASH_ADDR + FLASH_SIZE));
	...
	if (is_src_in_flash) {
		...
		if (tmp != NULL) {
			flash_nor_read_locked(src_addr, (uint8_t *)tmp, len);
			...
			LOG_ERR("k_malloc 0x%zx for flash data transfer station failed", len);
			return -ENOMEM;
		}
	}
#else
	__ASSERT(!is_src_in_flash, "not supported: Data in flash (0x%x) cannot be used as source",
		 src_addr);
...
static const struct flash_driver_api flash_rtl87x2g_driver_api = {
...
DEVICE_DT_INST_DEFINE(0, flash_rtl87x2g_init, NULL, &flash_data, NULL, POST_KERNEL,
