...
#define AES_ENCRYPTED_OUTPUT_SIZE \
	...
	start = timing_counter_get();
	status = psa_cipher_decrypt(key_id, PSA_ALG_ECB_NO_PADDING, out_buf, out_len,
...
static const uint8_t aes_mode_key[] = {
	0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
	0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
};

static const uint8_t aes_mode_iv[] = {
	0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
	0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
};

static const uint8_t aes_ctr_iv[] = {
...
static void test_aes_cipher_mode(const char *mode_name, psa_algorithm_t algorithm,
				 const uint8_t iv[16],
				 const uint8_t *ciphertext_ref,
				 size_t ciphertext_length)
{
	psa_key_attributes_t key_attr = PSA_KEY_ATTRIBUTES_INIT;
	psa_cipher_operation_t operation = PSA_CIPHER_OPERATION_INIT;
	psa_key_id_t key_id = PSA_KEY_ID_NULL;
	uint8_t ciphertext[PSA_CIPHER_ENCRYPT_OUTPUT_SIZE(
		PSA_KEY_TYPE_AES, PSA_ALG_CBC_PKCS7, sizeof(aes_mode_plaintext))] = {0};
	...
	timing_start();
	start = timing_counter_get();
	status = psa_cipher_update(&operation, aes_mode_plaintext,
				   sizeof(aes_mode_plaintext), ciphertext,
				   sizeof(ciphertext), &update_length);
	finish_length = 0U;
	...
	start = timing_counter_get();
	status = psa_cipher_update(&operation, ciphertext, ciphertext_length,
				   decrypted, sizeof(decrypted), &update_length);
	finish_length = 0U;
	if (status == PSA_SUCCESS) {
...
ZTEST_USER(psa_crypto_test_suite, test_aes_cbc_no_padding)
{
	...
	test_aes_cipher_mode("CBC-NO-PADDING", PSA_ALG_CBC_NO_PADDING, aes_mode_iv,
			     ciphertext_ref,
...
ZTEST_USER(psa_crypto_test_suite, test_aes_cbc_pkcs7)
{
	...
	test_aes_cipher_mode("CBC-PKCS7", PSA_ALG_CBC_PKCS7, aes_mode_iv,
			     ciphertext_ref,
			     sizeof(ciphertext_ref));
}
...
ZTEST_USER(psa_crypto_test_suite, test_ecc_ecdh)
{
	...
	timing_start();
	start = timing_counter_get();
	status = psa_raw_key_agreement(PSA_ALG_ECDH, key_id_a, public_key_b,
				       public_key_b_length, secret_a, sizeof(secret_a),
				       &secret_a_length);
	end = timing_counter_get();
	cycles = timing_cycles_get(&start, &end);
	nanoseconds = timing_cycles_to_ns(cycles);
	...
	status = psa_raw_key_agreement(PSA_ALG_ECDH, key_id_b, public_key_a,
				       public_key_a_length, secret_b, sizeof(secret_b),
				       &secret_b_length);
