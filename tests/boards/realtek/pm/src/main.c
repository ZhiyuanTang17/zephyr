...
static const uint32_t period_list_us[] = {20000,10000,9000, 8000,7000,6000,5000,3000,2500};// 2000,1875,1750,1625,1500,1375};//1250, 1000, 875, 750, 625, 500, 375, 250, 125};
...
static void timer_period_fn(struct k_timer *t)
{
	...
	if (pos < TOTAL_EXPIRE_CNT) {
		...
		log_buf[pos].low_power_mode_count = AON_REG_READ_BITFIELD(AON_REG_PCK600_AON_REG5X, VPON_LEAVE_FUNC_RET_MODE_CNT_VALUE);
		log_write_pos = pos + 1;
	}
...
ZTEST(pm_rtl8762j, test_timer)
{
	TC_PRINT("Start pm_rtl8762j test, start cycle:%llu\n", k_cycle_get_64());
	...
	for(curr_period_idx = 0; curr_period_idx < PERIOD_CNT; curr_period_idx++) {
		...
		last_exit_num = AON_REG_READ_BITFIELD(AON_REG_PCK600_AON_REG5X, VPON_LEAVE_FUNC_RET_MODE_CNT_VALUE);

		for (expire_cnt_in_period = 1; expire_cnt_in_period <= EXPIRE_PER_PERIOD; expire_cnt_in_period++) {
			last_cycle = k_cycle_get_64();
			k_timer_start(&periodic_timer, K_USEC(next_us), K_NO_WAIT);
			k_sem_take(&periodic_sem, K_FOREVER);
		}

		entered_dlps_times[curr_period_idx] = AON_REG_READ_BITFIELD(AON_REG_PCK600_AON_REG5X, VPON_LEAVE_FUNC_RET_MODE_CNT_VALUE) - last_exit_num;
	}
	...
	for (uint16_t i = 0; i < log_write_pos; i++) {
		...
		printk("Timer period:%u us, exp:%u, delta cycle:%llu, low_power_mode_count:%d\n",
		       l->period_us, l->expire_cnt, l->delta_cycle, l->low_power_mode_count);
	}

	for (uint16_t i = 0; i < PERIOD_CNT; i++) {
		printk("Timer period:%u us, exp count: %d, DLPS entry count:%d\n",
...
ZTEST_SUITE(pm_rtl8762j, NULL, NULL, before_fn, NULL, teardown_fn);
